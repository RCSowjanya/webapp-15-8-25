...
// Note: Avoid importing client-only libraries at module scope in server context
let toastRef = null;
const safeToast = async (type, message) => {
  try {
    if (typeof window !== "undefined") {
      ...
      if (toastRef) {
        if (type === "success") toastRef.success(message);
        else toastRef.error(message);
      }
    }
  } catch (_) {
    // ignore toast errors on server
  }
};
...
class ApiService {
  constructor() {
    ...
    console.log(
      ...
      Object.keys(process.env).filter((key) => key.includes("NEXT_PUBLIC"))
    );
  ...
  async apiCall({
    ...
    try {
      ...
      const finalHeaders = this.prepareHeaders(
        headers,
        requireAuth,
        authorizationHeader
      );
  ...
  async download(
    ...
    try {
      ...
      if (showErrorToast) {
        await safeToast("error", errorMessage);
      }
      throw error;
    }
  }
